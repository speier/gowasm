...
func RenderToString(n *VNode) string {
...
func render(w io.Writer, n *VNode) error {
	if x, ok := w.(writer); ok {
		return _render(x, n)
	}
...
func _render(w writer, n *VNode) error {
	if n.Type == TextNode {
	...
	if len(n.Children) > 0 {
		c := n.Children[0]
		if c != nil && c.Type == TextNode && strings.HasPrefix(c.TagName, "\n") {
			switch n.TagName {
